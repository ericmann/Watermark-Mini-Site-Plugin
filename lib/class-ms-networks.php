...
class MS_Networks {
	...
	/* Config Page*/
	public static function networks_page() {
...
?>
										...
										<?php else :?>
											<th scope="col">
												<a href="ms-admin.php?page=networks&sortby=<?php echo urlencode( $column_display_name ) ?>&<?php if( $_GET[ 'sortby' ] == $column_display_name ) { if( $_GET[ 'order' ] == 'DESC' ) { echo "order=ASC&" ; } else { echo "order=DESC&"; } } ?>start=<?php echo $start ?>"><?php echo $column_display_name; ?></a>
										<?php endif; ?>
											</th>
									<?php } ?>
								</tr>
...
?>
													<th scope="row" class="check-column" style="width: auto">
														<input type='checkbox' id='<?php echo $site[ 'id' ] ?>' name='allnetworks[]' value='<?php echo $site[ 'id' ] ?>'>
														<label for='<?php echo $site[ 'id' ] ?>'></label>
													</th>
<?php
												else :
?>
...
<?php
	...
	public static function move_site_page() {
		global $wpdb;

		if ( isset( $_POST['move'] ) && isset( $_GET['blog_id'] ) ) {
			if ( isset( $_POST['from'] ) && isset( $_POST['to'] ) ) {
				Watermark_Mini_Site::move_site( $_GET['blog_id'], $_POST['to'] );
				$_GET['updated'] = 'yes';
				$_GET['action'] = 'saved';
			}
		} else {
...
<?php
	...
	public static function reassign_site_page() {
		global $wpdb;
		...
		if ( isset( $_POST['reassign'] ) && isset( $_GET['id'] ) ) {
			...
			foreach( $sites as $site ) {
				Watermark_Mini_Site::move_site( $site, (int)$_GET['id'] );
			}
			...
			if ( ENABLE_NETWORK_ZERO ) {
				foreach( $current_blogs as $current_blog ) {
					if ( !in_array( $current_blog->blog_id, $sites ) )
						Watermark_Mini_Site::move_site( $current_blog->blog_id, 0 );
...
<?php endif; ?>
	...
	public static function update_network_page() {
		global $wpdb;
		...
		if ( isset( $_POST['update'] ) && isset( $_GET['id'] ) ) {
			...
			Watermark_Mini_Site::update_network( (int)$_GET['id'], $_POST['domain'], $_POST['path'] );
			$_GET['updated'] = 'true';
			$_GET['action'] = 'saved';
...
<?php
	...
	public static function delete_network_page() {
		global $wpdb;

		if ( isset( $_POST['delete'] ) && isset( $_GET['id'] ) ) {
			$result = Watermark_Mini_Site::delete_network( (int)$_GET['id'], ( isset( $_POST['override'] ) ) );
...
<?php
	...
	public static function delete_multiple_network_page() {

		global $wpdb;

		if(isset($_POST['delete_multiple']) && isset($_POST['deleted_networks'])) {
			foreach($_POST['deleted_networks'] as $deleted_network) {
				$result = Watermark_Mini_Site::delete_network((int)$deleted_network,(isset($_POST['override'])));
				if(is_a($result,'WP_Error')) {
					wp_die($result->get_error_message());
				}
			}
			...
			foreach($allnetworks as $network) {
				if(!Watermark_Mini_Site::network_exists((int)$network)) {
